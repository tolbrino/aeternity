     ...
     [{"Generate with a winning nonce and high target threshold, verify it",
       {timeout, 60,
        fun() ->
                ...
                Res = spawn_worker(fun() -> ?TEST_MODULE:generate(?TEST_BIN, Target, Nonce) end),
                {ok, {Nonce, Soln}} = Res,
      ...
      {"Generate with a winning nonce but low target threshold, shall fail",
       {timeout, 90,
        fun() ->
                Target = 16#01010000,
                Nonce = ?TEST_HIGH_NONCE,
                Res1 = spawn_worker(fun() -> ?TEST_MODULE:generate(?TEST_BIN, Target, Nonce) end),
                ...
                Res2 = spawn_worker(fun() -> ?TEST_MODULE:generate(?TEST_BIN, HighTarget, Nonce) end),
                {ok, {Nonce, Soln2}} = Res2,
      ...
      {"Attempt to verify nonce that does not have a solution (providing a dummy solution) shall fail",
       fun() ->
               ...
               ?assertMatch({error, no_solution},
                            spawn_worker(fun() -> ?TEST_MODULE:generate(?TEST_BIN, Target, Nonce) end)),
...
kill_ospid_miner_test_() ->
     [ {"Run miner in OS and kill it by killing parent",
       fun() ->
            Self = self(),
            Pid = spawn(fun() ->
                          Self ! {?TEST_MODULE:generate(?TEST_BIN, 12837272, 128253), self()}
                      end),
