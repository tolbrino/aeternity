...
factorial_contract(Config) ->
    ...
    %% Initialise contract owned by Alice.
    {EncodedContractPub,DecodedContractPub,_} =
       create_compute_contract(Node, APub, APriv, Code, <<"(0)">>),

    init_fun_calls(), % setup call handling

    %% Set worker contract. A simple way of pointing the contract to itself.
    call_func(APub, APriv, EncodedContractPub, <<"set_worker">>,
              args_to_binary([DecodedContractPub])),

    %% Compute fac(10) = 3628800.
    call_func(APub, APriv, EncodedContractPub,
              <<"fac">>, <<"(10)">>, {<<"int">>, 3628800}),
